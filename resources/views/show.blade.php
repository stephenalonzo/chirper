<x-layout>
    <div class="py-4">
        <div class="px-4 pb-4 flex flex-row items-start justify-between">
            ...
            @unless(count($followers) == 0 || count($following) == 0)
                @foreach ($followers as $follower)
                    @if ($follower->user_id == auth()->user()->id)
                        <a href="/users/{{ $user->id }}/unfollow"
                            class="px-4 py-2 rounded-md bg-gray-200 text-black">Unfollow</a>
                    @else
                        <a href="/users/{{ $user->id }}/follow"
                            class="px-4 py-2 rounded-md bg-blue-600 text-white">Follow</a>
                    @endif
                @endforeach
            @else
